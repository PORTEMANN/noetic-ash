#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""C18 — analyse de la campagne exp016 (attente EXP-016, expectations v0.8.0
...
Régénère signatures + verdicts (octets déterministes) depuis signaux/ :
  ...
  run/evaluation_clauses.json  — évaluation clause par clause du falsifieur EXP-016
...
F16 = next(a["falsifieur"] for a in exp["attentes"] if a["id"] == "EXP-016")
...
LETTRES = "spdfghik"
ETATS = [(f"{n}{LETTRES[l]}", n - l - 1) for n in range(1, 9) for l in range(n)]
PLUR = {0: "0 nœud", 1: "1 nœud"}
SN = ("Campagne exp016 C18 — orbitale radiale u_{{{}}}(r) sur fond monopole ρ=1 "
      "(C=1,3033), boîte RMAX=12000, NR=12000, dr=1")

TRAVAUX = [(f"p1_orb_{nom}", 12000, 1.0, 2.0**-11, 1024, 1024, "E",
            "p1_orbitale_radiale_n7_n8_boite12000", "spectre_lie",
            SN.format(nom) + f" ({PLUR.get(nd, str(nd) + ' nœuds')})", F16,
            ["noetic-machine-complete P1", "protocole C18 figé"])
           for nom, nd in ETATS]
TRAVAUX += [
    ("temoin_4f", 12000, 1.0, 2.0**-11, 1024, 1024, "H", "temoin_integrite",
     "spectre_lie", "Campagne exp016 C18 — témoin d'intégrité 4f boîte 12000 "
     "(même chaîne que C16, boîte étendue ; E=−1,664105e-6 Balmer-exact)", F_CTRL,
     ["protocole C18 figé"]),
    ("controle_bruit43", 2048, 10.0, 2.0**-11, 512, 256, "H", "controle_bruit",
     "etalonnage", "Campagne exp016 C18 — bruit blanc gaussien graine 43 "
...
for nom, npts, fs, f0, win, nps, statut, etiq, dom, desc, falsif, srcs in TRAVAUX:
    ...
    print(f"[C18] {nom:18s} Rc={s['invariants']['Rc']:12.6f} "
          f"ReN={s['invariants']['ReN']:10.6f} {s['regime']:13s} → {v['verdict']}")

# ---------------------------------------------------------------- clauses EXP-016
Rc = {f"{n}{LETTRES[l]}": resultats[f"p1_orb_{n}{LETTRES[l]}"]["Rc"]
      for n in range(1, 9) for l in range(n)}
# (a) ordre nodal sur les couches n=7 et n=8, tolérance 5 %
clA = True
quasi = []
detail_a = {}
for n in (7, 8):
    ...
    detail_a[str(n)] = {"ordre": [nm for nm, _ in seq], "Rc": rcs,
                        "inversions": inv}
# (b) ratios inter-couches à l fixe (séries n=l+1..8 d'états liés)
rats = {}
indef = []
for l in range(8):
    for n in range(max(1, l + 1), 8):
        cle = f"{n+1}{LETTRES[l]}/{n}{LETTRES[l]}"
        if Rc[f"{n}{LETTRES[l]}"] == 0.0:
            rats[cle] = None  # dénominateur nul : contenu spectral sous le plancher de grille
            indef.append(cle)
        else:
            rats[cle] = Rc[f"{n+1}{LETTRES[l]}"] / Rc[f"{n}{LETTRES[l]}"]
hors = {k: v for k, v in rats.items() if v is None or not (1.45 <= v <= 1.75)}
ndef = sum(1 for v in rats.values() if v is not None)
mg = math.exp(sum(math.log(v) for v in rats.values() if v is not None) / ndef)
clB = (not hors) and abs(mg / 1.5874 - 1) <= 0.05
# (c) toutes les orbitales mesurées Cosmologiques
clC = all(resultats[f"p1_orb_{n}{LETTRES[l]}"]["regime"] == "Cosmologique"
          for n in range(1, 9) for l in range(n))

# observation secondaire déclarée : ratios 8/7 des états à ratio Balmer < 0,5
energies = json.load(open(os.path.join(RUN, "energies_c18.json")))["energies"]
ALPHA, M_E = 1 / 137.036, 1.0
obs2 = {}
for nm in ("8s", "8p", "8d"):
    n = 8
    bal = -ALPHA**2 * M_E / (2 * n**2)
    rb = energies[nm] / bal
    obs2[f"{nm}/7{nm[1]}"] = {"ratio_Balmer_etat": rb,
                              "ratio_Rc_8_sur_7": Rc[nm] / Rc[f"7{nm[1]}"],
                              "attendu_diagnostic_boite": "< 1,45"}

clauses = {
    "attente": "EXP-016", "Rc": Rc,
    ...
    "clause_b_echelle": {"resultat": clB, "ratios": rats,
                         "ratios_indefinis_plancher": indef,
    ...
    "observation_secondaire_diagnostic_boite": obs2,
    "EXP016_conforme_aux_trois_clauses": clA and clB and clC,
}
with open(os.path.join(RUN, "evaluation_clauses.json"), "w", newline="") as f:
    f.write(json.dumps(clauses, ensure_ascii=False, indent=2) + "\n")
print("[C18] clauses : a =", clA, " b =", clB, " c =", clC)
...
for f in ("c18_runner.py", "c18_analyse.py", "ash_core.py", "noetic_bridge.py",
          "expectations_v080.json", "evaluation_clauses.json", "energies_c18.json"):
...
print(f"[C18] manifest.json : {len(man)} empreintes")
