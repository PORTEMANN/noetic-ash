#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""C23 — pipeline comma noétique figé (attente EXP-021, protocole C23).
Identique à C22 ; clause (b) reçoit la clause de domaine t_eff ≥ 130 apprise
d'EXP-020 (texte faisant foi : attente EXP-021, registre v1.3.0).

Clauses EXP-021 (les six graines doivent passer les clauses applicables) :
  (a) micro-rythme : sur une graine à résurgence avec ≥ 5 épisodes en
      ...
      2 DISTINGUABLES (test de permutation bilatéral sur l'écart des
      médianes, 10 000 tirages, graine 7 figée, p < 0,05) → falsification ;
      hors domaine si pas de résurgence ou acte 2 < 5 épisodes ;
  (b) robustesse Nv : sur une graine avec t_eff ≥ 130, le résidu de Nv
      présente ≥ 1 point Z < −3 ; hors domaine si t_eff < 130 ;
...
def main():
    ...
    for g in GRAINES:
        ...
        # clause (b) : Nv significatif, avec clause de domaine t_eff ≥ 130
        if res_nv["t_eff"] >= 130.0:
            clause_b = res_nv["n_significatifs"] > 0
            domaine_b = "applicable"
        else:
            clause_b = None
            domaine_b = f"HORS-DOMAINE (t_eff={res_nv['t_eff']:.1f} < 130)"
        ...
        toutes[str(g)] = {
            ...
            "clause_b_domaine": domaine_b,
            "clause_b_Nv_significatif": clause_b,
            "clause_c_Rdyn_borne": clause_c,
        }
        print(f"[C23] graine {g} : t_eff={res_nv['t_eff']:.1f}, "
              f"résurgence={er['resurgence']} (trou={er['trou_max']:.1f}, "
              f"reprise={er['reprise']}), bande Lt={len(res_lt['bandes'])>0}, "
              f"Nv Zmin={res_nv['z_min']:.2f}, Rdyn={rid['Rdyn']:.4f}", flush=True)
        if detail_a:
            print(f"[C23]   (a) distinguabilité : |Δmed|={tp['ecart_medianes']:.2f}, "
                  f"p={tp['p_value']:.4f} (n1={tp['n1']}, n2={tp['n2']}) → "
                  f"{'indistinguables, tenue' if clause_a else 'DISTINGUABLES, falsifiée'}",
                  flush=True)
        else:
            print(f"[C23]   (a) {domaine_a}", flush=True)
        print(f"[C23]   (b) {domaine_b} → {clause_b} ; (c) {clause_c}", flush=True)

    conforme = all((toutes[g]["clause_a_micro_rythme"] is not False)
                   and (toutes[g]["clause_b_Nv_significatif"] is not False)
                   and toutes[g]["clause_c_Rdyn_borne"] for g in toutes)
    n_a = sum(1 for g in toutes if toutes[g]["clause_a_micro_rythme"] is not None)
    evaluation = {
        "attente": "EXP-021",
        "graines": GRAINES,
        "par_graine": toutes,
        "n_graines_clause_a_applicables": n_a,
        "EXP021_conforme_sur_les_six_graines": conforme,
    }
    ...
    print(f"[C23] clause (a) applicable sur {n_a}/6 graines ; "
          f"EXP-021 {'CONFORME' if conforme else 'FALSIFIÉE'}")
    ...
    print(f"[C23] manifest.json : {len(man)} empreintes")
