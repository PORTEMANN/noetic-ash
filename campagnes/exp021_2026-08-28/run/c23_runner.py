#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""C23 — campagne exp021 : micro-rythme de la résurgence E44, six graines
aveugles 2035–2040 (attente EXP-021, figée AVANT mesure,
bridge/expectations.json v1.3.0).

Régénère les six graines (reprise exacte du volet a de c14_runner.py :
e44_core.py, SeedSequence(2035..2040), N=48, A=1, gamma=0.2, dt=0.1,
2048 pas). Témoins d'intégrité : les graines 2028 et 2029 doivent être
octet-identiques aux campagnes exp017/exp018.

Produit (octets déterministes, fins de ligne LF) :
  signaux/e44_{nv,lt}_t_g{2035..2040}.csv (12 fichiers)
  run/manifest_signaux.json

Usage : python3 run/c23_runner.py   (depuis la racine de la campagne)
"""
...
for g in GRAINES:
    ...
    print(f"[C23] graine {g} : sha256 tableaux nus = "
          f"{hashlib.sha256(Nv.tobytes() + Lt.tobytes()).hexdigest()}", flush=True)
...
print(f"[C23] manifest_signaux.json : {len(man)} empreintes")
